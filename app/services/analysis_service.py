...
from app.engines.payloads import PayloadLibrary


class VulnAnalyzer:
    """Analizador de vulnerabilidades potenciado por IA."""

    def __init__(self, payload_library: Optional[PayloadLibrary] = None) -> None:
        """
        Inicializa el analizador.
        
        Args:
            payload_library: Librería de payloads opcional.
        """
        self.payloads = payload_library or PayloadLibrary()
    ...
    def _check_sensitive_directories(
        ...
        for directory in directories:
            url_full = directory.get("url", "").lower()
            path = urlparse(url_full).path
            status = directory.get("status", 0)

            for pattern in sensitive_patterns:
                if pattern in path and status in (200, 301, 302, 403):

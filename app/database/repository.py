...
class ScanRepository:
    ...
    async def add_agent_thought(
        ...
        """Guarda un pensamiento táctico del agente de IA."""
        return await db.fetch_val(
            "INSERT INTO agent_thoughts (scan_id, thought, proposed_action, priority) "
            "VALUES ($1, $2, $3, $4) RETURNING id",
            scan_id, thought, proposed_action, priority
        )
    ...
    async def create_pending_action(
        ...
        """Registra una acción que requiere aprobación humana."""
        return await db.fetch_val(
            "INSERT INTO pending_actions (scan_id, action_type, command, reason_thought) "
            "VALUES ($1, $2, $3, $4) RETURNING id",
            scan_id, action_type, command, reason_thought
        )

    async def get_pending_actions(self, scan_id: int) -> list[dict[str, Any]]:
        """Obtiene las acciones pendientes de aprobación para un escaneo."""
        rows = await db.fetch_all(
            "SELECT * FROM pending_actions WHERE scan_id = $1 AND status = 'pending' "
    ...
    async def update_pending_action_status(
        ...
        """Actualiza el estado de una acción (approved/rejected)."""
        await db.execute(
            "UPDATE pending_actions SET status = $1, resolved_at = CURRENT_TIMESTAMP "
            "WHERE id = $2",
            status, action_id
        )
